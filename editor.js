...
function showWelcome() {
  state.selected=null;
  panel.innerHTML=`<h1>Click anything to edit</h1><p>Click text in the preview and type. Click a project, category, service, image, or section for more controls.</p><h2>Contact information</h2>${inputField("Phone",state.content?.contact.phone||"","contact.phone")}${inputField("Phone link",state.content?.contact.phoneUrl||"","contact.phoneUrl")}${inputField("WhatsApp link",state.content?.contact.whatsappUrl||"","contact.whatsappUrl")}${inputField("Email",state.content?.contact.email||"","contact.email","email")}${inputField("LinkedIn",state.content?.contact.linkedinUrl||"","contact.linkedinUrl")}${inputField("Location",state.content?.contact.location||"","contact.location")}${inputField("Map link",state.content?.contact.locationUrl||"","contact.locationUrl")}<div class="global-settings"><h2>Whole website style</h2><label>Body font<select data-global="site.fontFamily"><option value='Inter, ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'>Inter / clean sans</option><option value="Arial, Helvetica, sans-serif">Arial</option><option value="Georgia, serif">Georgia</option></select></label><label>Accent font<select data-global="site.accentFontFamily"><option value="Georgia, serif">Georgia</option><option value="Inter, ui-sans-serif, sans-serif">Inter</option></select></label><label>Base font size<input type="number" min="12" max="24" data-global="site.baseFontSize"></label><div class="colors"><label>Main background<input type="color" data-global="site.paperColor"></label><label>Dark color<input type="color" data-global="site.darkColor"></label><label>Highlight<input type="color" data-global="site.primaryColor"></label><label>Accent<input type="color" data-global="site.accentColor"></label></div></div>`;
